...
Inheritance & Polymorphism: Implemented a parent Bank class with specialized Savingacc and Currentacc subclasses. 

Method Overriding: Custom withdrawal logic for different account types (e.g., Overdraft limit for Current Accounts).

Thread Safety (Concurrency): Used the synchronized keyword to ensure data consistency when multiple users access the same account simultaneously. 
